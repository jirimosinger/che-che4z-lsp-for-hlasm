...
external_resource SOURCE_FILE_RES("OPEN");
...
TEST_F(lsp_features_test, go_to)
{
    ...
    EXPECT_EQ(location(position(1, 7), MACRO_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(0, 4)));
    // no jump
    EXPECT_EQ(location(position(0, 8), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(0, 8)));
    // jump in source, open code, var symbol &VAR
    EXPECT_EQ(location(position(1, 0), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(2, 13)));
    // jump in source, open code, seq symbol .HERE
    EXPECT_EQ(location(position(5, 0), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(3, 13)));
    // jump in source, macro, seq symbol .HERE
    EXPECT_EQ(
        location(position(15, 0), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(12, 15)));
    // jump in source, macro, var symbol &LABEL
    EXPECT_EQ(
        location(position(8, 0), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(11, 20)));
    // jump in source, macro, var symbol &VAR
    EXPECT_EQ(
        location(position(8, 13), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(11, 15)));
    // forward jump in source, open code, ord symbol R1
    EXPECT_EQ(
        location(position(22, 0), SOURCE_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(21, 10)));
    // jump from source to copy file, ord symbol R2 on machine instruction
    EXPECT_EQ(location(position(0, 0), COPY_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(21, 13)));
    // jump from source to copy file, ord symbol R2 on macro MAC
    EXPECT_EQ(location(position(0, 0), COPY_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(23, 14)));
    // jump from source to first instruction in copy file, COPY COPYFILE
    EXPECT_EQ(location(position(0, 3), COPY_FILE), a.context().lsp_ctx->definition(SOURCE_FILE_RES, position(20, 14)));
}
