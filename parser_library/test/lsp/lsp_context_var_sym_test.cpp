...
{
    auto res = a.context().lsp_ctx->hover(opencode_file, { 2, 7 });
...
{
    auto res =
        a.context().lsp_ctx->completion(opencode_file, { 3, 2 }, '&', completion_trigger_kind::trigger_character);
...
{
    auto res = a.context().lsp_ctx->hover(opencode_file, { 0, 6 });
...
{
    auto res = a.context().lsp_ctx->references(opencode_file, { 0, 9 });
    ASSERT_EQ(res.size(), 0U);
}

TEST_F(lsp_context_var_symbol_no_definition, hover_no_occurence)
{
    auto res = a.context().lsp_ctx->hover(opencode_file, { 0, 9 });
