...
{
    location res = a.context().lsp_ctx->definition(opencode_file_res, { 4, 12 });
...
{
    location res = a.context().lsp_ctx->definition(opencode_file_res, { 11, 12 });
    EXPECT_EQ(res.file, opencode_file_res);
...
{
    auto res = a.context().lsp_ctx->references(opencode_file_res, { 4, 12 });

    ASSERT_EQ(res.size(), 2U);

    EXPECT_EQ(res[0].file, opencode_file_res);
    EXPECT_EQ(res[0].pos, position(4, 11));
    EXPECT_EQ(res[1].file, opencode_file_res);
    EXPECT_EQ(res[1].pos, position(5, 0));
}

TEST_F(lsp_context_seq_sym, references_out_of_macro)
{
    auto res = a.context().lsp_ctx->references(opencode_file_res, { 11, 12 });
...
{
    auto res = a.context().lsp_ctx->hover(opencode_file_res, { 4, 12 });
...
{
    auto res = a.context().lsp_ctx->completion(opencode_file_res, { 6, 1 }, '\0', completion_trigger_kind::invoked);
...
{
    auto res = a.context().lsp_ctx->completion(opencode_file_res, { 13, 1 }, '\0', completion_trigger_kind::invoked);
...
{
    location res = a.context().lsp_ctx->definition(opencode_file_res, { 14, 12 });
    EXPECT_EQ(res.file, opencode_file_res);
...
{
    auto res = a.context().lsp_ctx->references(opencode_file_res, { 14, 12 });
