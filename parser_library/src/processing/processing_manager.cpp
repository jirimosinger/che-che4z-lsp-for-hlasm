    ...
    , stms_analyzers_({ &lsp_analyzer_ })
{
    switch (data.proc_kind)
    {
        ...
        case processing_kind::COPY:
            start_copy_member(copy_start_data { data.library_member, std::move(resource) });
            break;
        case processing_kind::MACRO:
            start_macro_definition(macrodef_start_data(data.library_member), std::move(resource));
            break;
        default:
            break;
    }
...
{
    hlasm_ctx_.push_statement_processing(processing_kind::COPY, std::move(start.member_resource));
...
void processing_manager::start_macro_definition(
    macrodef_start_data start, std::optional<utils::path::external_resource> resource)
{
    if (resource)
        hlasm_ctx_.push_statement_processing(processing_kind::MACRO, std::move(*resource));
    else
