...
void workspace_manager::did_change_watched_files(const char** paths, size_t size)
{
    ...
    {
        paths_s.push_back(utils::path::external_resource(paths[i]));
    }
...
{
    impl_->did_open_file(utils::path::external_resource(document_uri), version, std::string(text, text_size));
}
...
{
    impl_->did_change_file(utils::path::external_resource(document_uri), version, changes, ch_size);
}

void workspace_manager::did_close_file(const char* document_uri)
{
    impl_->did_close_file(utils::path::external_resource(document_uri));
}
