...
using list_directory_result = std::pair<std::unordered_map<std::string, utils::path::external_resource>, utils::path::list_directory_rc>;
...
public:
    ...
    virtual file_ptr add_file(const utils::path::external_resource&) = 0;

    // Adds processor file with specified file name and returns it.
    // If such processor file already exists, it is returned.
    // If such file already exists, it is changed into processor file.
    virtual processor_file_ptr add_processor_file(const utils::path::external_resource&) = 0;
    virtual processor_file_ptr get_processor_file(const utils::path::external_resource&) = 0;
    ...
    virtual processor_file_ptr find_processor_file(const utils::path::external_resource& key) = 0;
    ...
    virtual void did_open_file(
        const utils::path::external_resource& document_uri, version_t version, std::string text) = 0;
    virtual void did_change_file(const utils::path::external_resource& document_uri,
        version_t version,
        const document_change* changes,
        size_t ch_size) = 0;
    virtual void did_close_file(const utils::path::external_resource& document_uri) = 0;
