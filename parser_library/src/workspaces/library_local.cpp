...
void library_local::load_files()
{
    ...
    switch (rc)
    {
        ...
        case hlasm_plugin::utils::path::list_directory_rc::not_exists:
            if (!optional_)
                add_diagnostic(diagnostic_s::error_L0002(lib_uri_.get_absolute_path()));
            break;
        ...
        case hlasm_plugin::utils::path::list_directory_rc::other_failure:
            add_diagnostic(diagnostic_s::error_L0001(lib_uri_.get_absolute_path()));
            break;
    }
        ...
        for (const auto& extension : extensions_)
        {
            ...
            if (!inserted)
                add_diagnostic(diagnostic_s::warning_L0004(
                    lib_uri_.get_absolute_path(), context::to_upper_copy(std::string(filename))));
    ...
    if (extension_removed && extensions_from_deprecated_source)
        add_diagnostic(diagnostic_s::warning_L0003(lib_uri_.get_absolute_path()));
